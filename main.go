...
func (t Training) distance() float64 {
	...
	return float64(t.Action) * t.LenStep / float64(MInKm) // (км)
}

// meanSpeed возвращает среднюю скорость бега или ходьбы.
func (t Training) meanSpeed() float64 {
	// вставьте ваш код ниже
	return (float64(t.Action) * t.LenStep / float64(MInKm)) / t.Duration.Hours() // (км/ч)
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	c := (CaloriesWeightMultiplier*w.Weight + (math.Pow(KmHInMsec*w.meanSpeed(), 2)/(w.Height/100))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
	return c
}
